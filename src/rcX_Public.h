...
*******************************************************************************
  ...
  Last Modification:
    @version $Id: rcX_Public.h 348 2010-04-28 05:08:47Z MichaelT $
  ...
  ---------------------------------------------------------------------------
    2010-04-28  MT            Removed define CIFX_TOOLKIT and included common
                              packing __TLR_PACKED_PRE / POST
    2010-04-27  VD            For CIFX_TOOLKIT: Changed defines
                              __TKITPACKED_xx from __PACKED_xx to
                              __TLR_PACKED_xx

                              Changed include of TLR_INX_Includes.h to
                              TLR_Types.h

    2010-04-20  bm            added new packet definition
                              RCX_SET_HANDSHAKE_CONFIG_REQ

    2010-04-16  mt            RCX_VERIFY_DATABASE_REQ_SIZE did not work due
                              to an additional space character

    2010-04-13  NC            Included TLR_INX_Includes.h instead of
                              TLR_Includes.h
...
{
  TLR_UINT8 abData[2008]; /* info field data */
...
typedef __TLR_PACKED_PRE struct RCX_TSK_GET_INFO_ARRAY_CNF_DATA_Ttag
{
  ...
  RCX_TSK_GET_INFO_ARRAY_CNF_DATA_ELEMENT
  atInfoData[(2048 - sizeof(TLR_PACKET_HEADER_T)) / sizeof(RCX_TSK_GET_INFO_ARRAY_CNF_DATA_ELEMENT)];
...
#define RCX_NUM_QUEUES_PER_PACKET 502

typedef __TLR_PACKED_PRE struct RCX_QUE_GET_LOAD_REQ_DATA_Ttag
{
  TLR_UINT32 ulQue[RCX_NUM_QUEUES_PER_PACKET]; /* queue entries */
...
{
  TLR_UINT32 ulQueLoad[RCX_NUM_QUEUES_PER_PACKET]; /* queue get load entries */
...
typedef __TLR_PACKED_PRE struct RCX_TSK_GET_STATUS_ARRAY_CNF_DATA_Ttag
{
  ...
  TLR_UINT32 ulNumberOfEntries; /* number of entries (declaring range of start to start+count-1) */
  TLR_UINT32 ulCurrentTskCount; /* current number of tasks */
  TLR_UINT32 aulStatusCodes[(2048 - sizeof(TLR_PACKET_HEADER_T)) / sizeof(TLR_UINT32) - 3];
...
#define RCX_FILE_XFER_FILESYSTEM \
  1 /* transfer is related to the Filesystem installed in the Firmware (i.e. 2nd stage bootloader etc.) */
#define RCX_FILE_XFER_FILE RCX_FILE_XFER_FILESYSTEM
#define RCX_FILE_XFER_MODULE \
  2 /* will be directly loaded into ram and relocated to be integrated in the running firmware */
...
typedef __TLR_PACKED_PRE struct RCX_BACKUP_REQ_DATA_Ttag
{
  TLR_CHAR szBackupPoint[1]; /*!< NULL terminated string containing the path
                                                                           to backup to. This is just a placeholder, and
                                the strucure needs to be extended by the number of characters of the backup point, e.g.
                                "SDMMC:/backup" */
...
typedef __TLR_PACKED_PRE struct RCX_RESTORE_REQ_DATA_Ttag
{
  TLR_CHAR szRestorePoint[1]; /*!< NULL terminated string containing the path
                                                                              to restore from. This is just a
                                 placeholder, and the strucure needs to be extended by the number of characters of the
                                 restore point e.g.  "SDMMC:/backup" */
